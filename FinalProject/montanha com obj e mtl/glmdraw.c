...
void glmMakeVBOs(GLMmodel *model){
...
void glmLoadTextures(GLMmodel *model){
   uint i;
...
void glmDrawVBO(GLMmodel *model) {
   GLMgroup* group;
   uint prevMaterial = ~0;
...
void glmShaderMaterial(GLMmaterial *mat){
   ...
   static const float diffuseLight[4] = { 0.7, 0.7, 0.7, 1.0 };
...
void glmSpecularTexture(GLMmodel *model, uint cubeTex){
   uint i;
   for (i = 0; i < model->nummaterials; i++) {
